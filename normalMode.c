...
extern unsigned int bg[], fg, currentBg, highlightBg, highlightFg, amountNmKeys, vimSearchFg, vimSearchBg;
...
ExitState kpressHist(char const *cs, int len, int ctrl, KeySym const *ksym) {
	...
	if (!posLin || posLin==h || !h) strcpy(posBuffer, " [TOP] ");
...
void historyOverlay(int x, int y, Glyph* g) {
	...
	if(overlay && term.col > 9 && term.row > 4 && (x > (2*term.col/3)) && (y >= (term.row-1))) {
		*g = styleSearch;
		if (y) getChar(&searchStr, g, term.row-1, term.col-1, term.col/3, x);
	} else if (highlighted(x, y)) g->bg = vimSearchFg, g->fg = vimSearchBg;
